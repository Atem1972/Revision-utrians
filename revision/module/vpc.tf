...
resource "aws_subnet" "public" {
  ...
  availability_zone = var.availability_zone[count.index]
  tags = {
    "Name" = "Application-lb-public"
  }
}

# create a route table
resource "aws_route_table" "this-rt1" {
  vpc_id = aws_vpc.this.id
  tags = {
    "Name" = "Application-lb-route-table"
  }
...
# create a route table association
resource "aws_route_table_association" "private" {
  count          = length(var.subnet_cidr_public)  # this mean go to that variable count the total number of element in the value and creat me route-table-association of that same number
  subnet_id      = element(aws_subnet.public.*.id, count.index) # it will go to that particular variable retrieve the element(value) and bring it here . and .*.id means add .id at the end 
  route_table_id = aws_route_table.this-rt1.id                     #count.index means differential the two table name by add 0 to the first route-table-associate name and 1 to the second
}